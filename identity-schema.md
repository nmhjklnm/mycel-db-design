...
-- password_hash / api_key_hash：仅列级 GRANT，不暴露给 authenticated
-- ================================================================
...
-- api_key_enc：服务端 AES-256-GCM 加密，仅列级 GRANT，不暴露给 authenticated
...
-- 列级 GRANT：只开放安全列，password_hash / api_key_hash 不在列表中
-- 注意：PostgreSQL 中 REVOKE SELECT(col) 无法撤销表级 GRANT SELECT，
--       必须用列级 GRANT 替代表级 GRANT，才能真正屏蔽敏感列
GRANT SELECT (id, user_id, username, created_at, updated_at)
    ON identity.accounts TO authenticated;
```
...
-- 列级 GRANT：只开放安全列，api_key_enc 不在列表中
-- 注意：PostgreSQL 中 REVOKE SELECT(col) 无法撤销表级 GRANT SELECT，
--       必须用列级 GRANT 替代表级 GRANT，才能真正屏蔽 api_key_enc
GRANT SELECT (id, user_id, name, provider_type, base_url, status, last_check_at, last_error, created_at, updated_at)
    ON identity.model_providers TO authenticated;
...
|------|-------|----------|--------|---------------|--------------|-----------------|----------------|
| `anon` | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| `authenticated` | SELECT all rows / UPDATE(display_name,avatar,bio) own | SELECT(id,user_id,username,created_at,updated_at) own | SELECT own + avatar kind / INSERT+UPDATE(deleted_at) own | ALL own | SELECT own-created/used | SELECT(id,user_id,name,provider_type,base_url,status,last_check_at,last_error,created_at,updated_at) own / UPDATE(name,base_url,status) / DELETE | SELECT/INSERT/UPDATE(non-active)/DELETE own |
...
**对策**：
1. 列级 `GRANT SELECT (id, user_id, name, ...)` — 不含 api_key_enc，DB 层彻底屏蔽（PostgreSQL 中 REVOKE SELECT(col) 无法撤销表级 GRANT，必须用列级 GRANT 替代）
...
**对策**：
1. 列级 `GRANT SELECT (id, user_id, username, created_at, updated_at)` — 不含 password_hash / api_key_hash，DB 层彻底屏蔽（PostgreSQL 中 REVOKE SELECT(col) 无法撤销表级 GRANT，必须用列级 GRANT 替代）

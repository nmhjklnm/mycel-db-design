...
-- ================================================================
-- 2. container.sandbox_templates
--    可复用的沙盒创建蓝图。用户创建 sandbox 时选择模板并覆盖参数。
--    system 模板（owner_user_id IS NULL）对所有用户可见。
-- ================================================================

CREATE TABLE container.sandbox_templates (
    id            TEXT        PRIMARY KEY DEFAULT gen_random_uuid()::text,
    owner_user_id TEXT,
        -- → identity.users（应用层）；NULL = 系统公共模板
    name          TEXT        NOT NULL,
        -- e.g. "Python 3.12 + CUDA 12", "Node.js 22 minimal"
    provider_name TEXT,
        -- 限定适用 provider；NULL = 通用
        -- 'docker' | 'e2b' | 'fly' | 'local' | NULL
    image         TEXT,
        -- 基础镜像 e.g. "ubuntu:24.04", "python:3.12-slim"
    resource_spec JSONB       NOT NULL DEFAULT '{}',
        -- {"cpus": 4, "ram_gb": 8, "gpu": "A100", "disk_gb": 20}
    env_vars      JSONB       NOT NULL DEFAULT '{}',
        -- 预设环境变量 {"PATH": "...", "CUDA_HOME": "..."}
    init_script   TEXT,
        -- sandbox 启动后执行的初始化脚本
    ...
    CONSTRAINT sandbox_templates_status_chk
        CHECK (status IN ('active', 'deprecated'))
);

CREATE INDEX idx_sandbox_templates_owner
    ON container.sandbox_templates(owner_user_id)
    WHERE owner_user_id IS NOT NULL;

CREATE INDEX idx_sandbox_templates_public
    ON container.sandbox_templates(name)
    WHERE owner_user_id IS NULL AND status = 'active';


-- ================================================================
-- 3. container.sandboxes
--    设备上的隔离沙盒执行环境（原 staging.sandbox_instances 重构）
--    注：32-schema-decisions.md 中此表名为 containers；
--        后统一改名为 sandboxes，以避免 container.containers 同名混淆。
-- ================================================================
...
CREATE TABLE container.sandboxes (
    ...
    device_id        TEXT        NOT NULL,
        -- → container.devices.id（应用层）；沙盒属于设备
    owner_user_id    TEXT        NOT NULL,
    ...
    provider_env_id  TEXT,
        ...
        -- 同设备下同提供商 sandbox 不重复注册（见 uq_sandboxes_device_provider）
    image            TEXT,
        -- 基础镜像，e.g. "ubuntu:24.04" / "python:3.12-slim"
        -- local 类型可为空
    -- 创建来源（可选）
    template_id       TEXT,
        -- → container.sandbox_templates.id（应用层）；手动创建时为 NULL
    config            JSONB       NOT NULL DEFAULT '{}',
        -- 实际生效的参数（template 默认值 + 用户覆盖后的最终态）
        -- {"image": "...", "cpus": 4, "ram_gb": 8, "gpu": null, "env_vars": {...}}
        -- 创建后固化，不随 template 变更
    ...
    -- 值域约束
    CONSTRAINT sandboxes_desired_state_chk
        CHECK (desired_state IN ('running', 'stopped')),
    CONSTRAINT sandboxes_observed_state_chk
        CHECK (observed_state IN ('running', 'stopped', 'creating', 'detached', 'error')),
    CONSTRAINT sandboxes_status_chk
        CHECK (status IN ('active', 'archived', 'deleted'))
);

-- 同设备下同提供商沙盒不重复注册
CREATE UNIQUE INDEX uq_sandboxes_device_provider
    ON container.sandboxes(device_id, provider_env_id)
    WHERE provider_env_id IS NOT NULL;

-- 设备的所有沙盒（最常用：展示设备下沙盒列表）
CREATE INDEX idx_sandboxes_device
    ON container.sandboxes(device_id)
    WHERE status = 'active';

-- 用户的所有活跃沙盒（RLS / 跨设备查询）
CREATE INDEX idx_sandboxes_owner_active
    ON container.sandboxes(owner_user_id)
    WHERE status = 'active';

-- 状态不一致的沙盒（控制面轮询：desired ≠ observed 需要协调）
CREATE INDEX idx_sandboxes_state_mismatch
    ON container.sandboxes(updated_at)
    WHERE desired_state != observed_state AND status = 'active';


-- ================================================================
-- 4. container.workspaces
--    沙盒内的项目工作区（原 staging.sandbox_leases 重构）
...
CREATE TABLE container.workspaces (
    ...
    sandbox_id       TEXT        NOT NULL,
        -- → container.sandboxes.id（应用层）；工作区在沙盒内
    owner_user_id    TEXT        NOT NULL,
...
-- 同沙盒下路径唯一（排除已删除，允许路径复用）
CREATE UNIQUE INDEX uq_workspaces_sandbox_path
...
-- 状态不一致的工作区（控制面轮询，与 sandboxes 同模式）
...
> 2026-04-13，针对 devices / sandboxes / workspaces 三张表
...
ALTER TABLE container.sandboxes    ENABLE ROW LEVEL SECURITY;
...
```sql
-- SELECT：只能看自己的沙盒
CREATE POLICY sandboxes_select_own ON container.sandboxes
    FOR SELECT TO authenticated
    USING (owner_user_id = (SELECT auth.uid()::text));

-- INSERT：前端不能直接创建沙盒
-- 沙盒由 mycel-container 后端创建（需要验证 device 归属 + 调用 provider API）
-- 不创建 INSERT policy = 默认拒绝

-- UPDATE：前端只能改 desired_state（用户点启动/停止）和 name
-- observed_state / version / last_error / last_seen_at 由后端写
CREATE POLICY sandboxes_update_own ON container.sandboxes
    FOR UPDATE TO authenticated
    USING (owner_user_id = (SELECT auth.uid()::text))
    WITH CHECK (owner_user_id = (SELECT auth.uid()::text));

GRANT SELECT ON container.sandboxes TO authenticated;
-- UPDATE 只开放意图字段
GRANT UPDATE (name, desired_state)
    ON container.sandboxes TO authenticated;
...
```sql
-- 替换上面的 sandboxes_update_own，加 status 检查
DROP POLICY IF EXISTS sandboxes_update_own ON container.sandboxes;

CREATE POLICY sandboxes_update_own ON container.sandboxes
    FOR UPDATE TO authenticated
    USING (
        owner_user_id = (SELECT auth.uid()::text)
        AND status = 'active'           -- 不能操作已归档/删除的沙盒
    )
    WITH CHECK (
        owner_user_id = (SELECT auth.uid()::text)
        AND status = 'active'
...
-- INSERT：前端可以在自己的沙盒里创建工作区
-- 需要验证 sandbox_id 确实属于当前用户（防止越权关联他人沙盒）
CREATE POLICY workspaces_insert_own ON container.workspaces
    FOR INSERT TO authenticated
    WITH CHECK (
        owner_user_id = (SELECT auth.uid()::text)
        AND EXISTS (
            SELECT 1 FROM container.sandboxes s
...
| `container.sandboxes` | ✅ 必须 | INSERT, UPDATE | 沙盒创建进度 + 状态机变更（starting → running / error） |
...
ALTER PUBLICATION supabase_realtime
    ADD TABLE container.devices, container.sandboxes, container.workspaces;
```
...
#### container.sandboxes — INSERT + UPDATE

```sql
-- INSERT：用户发起创建后，后端异步建沙盒，前端监听创建完成
-- UPDATE：observed_state 变化（detached→creating→running / error）
-- Supabase JS:
-- supabase
--   .channel('my-sandboxes')
--   .on('postgres_changes', {
--       event: '*',                          -- INSERT + UPDATE
--       schema: 'container',
--       table: 'sandboxes',
...
- 关键 UX：用户点"启动沙盒"后页面不轮询，靠 Realtime 推 observed_state='running' 才解锁操作
...
| 角色 | devices | sandboxes | workspaces |
|------|---------|-----------|------------|
| `anon` | ❌ 无任何访问 | ❌ | ❌ |
...
- workspaces INSERT 额外校验 sandbox 归属（防止关联他人沙盒）
...
- `REVOKE UPDATE (provider_env_id, provider_name) ON container.sandboxes FROM authenticated`
...
### RPC 2：container.sandbox_observe

daemon 上报 sandbox 实际状态，后端读回 desired_state 作为指令。

```sql
CREATE OR REPLACE FUNCTION container.sandbox_observe(
    p_sandbox_id    TEXT,
    p_version       INTEGER,
...
与 sandbox_observe 同模式，workspace 独立上报。
...
daemon 断开连接时原子级联：设备→offline，其下所有沙盒→detached。
...
BEGIN
    ...
    -- 该设备下所有活跃沙盒标记 observed_state = 'detached'
    ...
    -- 工作区同步标记 detached（通过 sandbox_id 关联）
    UPDATE container.workspaces w
    SET
        observed_state = 'detached',
        version        = w.version + 1,
        updated_at     = now()
    FROM container.sandboxes s
    WHERE w.sandbox_id  = s.id
...
RETURNS TABLE(
    entity_type TEXT,   -- 'sandbox' | 'workspace'
    entity_id   TEXT,
    device_id   TEXT,
    desired     TEXT,
    observed    TEXT,
...
AS $$
    ...
    JOIN container.sandboxes s ON w.sandbox_id = s.id
...
3. **workspaces INSERT RLS 的 subquery 性能**：INSERT policy 中用了 `EXISTS (SELECT 1 FROM container.sandboxes ...)`，高频创建时需要确认 sandboxes(id, owner_user_id) 有覆盖索引（当前设计已有 `idx_sandboxes_owner_active`，可覆盖）。

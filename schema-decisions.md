...
1. **三层模型 device → sandbox → workspace** — 替代旧 lease/instance 二层
2. **thread ↔ workspace 连接关系** — agent.threads.current_workspace_id，可切换，非 1:1 绑定
3. **DB vs 用户命名** — DB: devices/sandboxes/workspaces；用户: 设备/工作区
...
assets 的 FK 指向 users.id，存的是用户主动管理的静态资源（头像、文件）。agent 产出物不进这里，直传 Supabase Storage（文件上传下载不穿过 DB 抽象层）。assets 和 identity 绑定紧密。
